...
spid[i]$sequence
PSMatch::calculateFragments("THSQEEMQHMQR")
mz(spid[i])[[1]]
```
...
| MS2 | Counting in DDA (integer) / Fragment XIC in DIA (Decimal) | TMT (iTRAQ) (Decimal) |
...
rowData(feat1[[1]])
```
If you directly use the output of MaxQuant, PD or sage, you do not need the following BUT you will miss the **smart** filtering of data > REMEMBER TO DO THIS IN CAPSTONE PROJECT
...
colData(cptac_se)
## Contaminant database/fasta: CRAP, you can download it
```
...
| Type | Assumption about missing values | Purpose of imputation | Key nuance | Typical methods | Proteomics context | Bottom line |
|------|--------------------------------|------------------------|------------|------------------|--------------------|-------------|
| MAR  | Similar to observed values     | Estimate plausible values from existing data | Reconstructs the original distribution | KNN, mean/median, regression | Random missingness (e.g. stochastic sampling in DDA, some DIA gaps) | Recover what was likely there |
| MNAR | Systematically different (e.g. low abundance) | Model missingness mechanism (e.g. detection limit) | Introduces a shifted/bias distribution | Left-shifted Gaussian, minimum value imputation | Low-abundance peptides below detection (common in DDA) | Reflect why it was not observed |
...
#Group task (which contains most decoys)
...
data.frame(id) |> as_tibble() |> ggplot(aes(x = MetaMorpheus.score, colour = isDecoy)) +
    geom_density() + facet_wrap(~ file)
