...
# Output Directory
out_dir <- "/Users/mikhaila/Desktop/MSProt2026/team2/optional_analysis/plots_final"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

parquet_file <- "/Users/mikhaila/Desktop/MSProt2026/team2/identification/report.parquet"
fasta_file <- "/Users/mikhaila/Desktop/MSProt2026/team2/identification/contaminants.fasta"
...
data_precursor <- data %>%
  dplyr::filter(Proteotypic == TRUE) %>%
  dplyr::select(Precursor.Id, Run, Precursor.Normalised, Protein.Ids) %>%
  dplyr::collect()
...
annotation <- data %>%
  dplyr::filter(Proteotypic == TRUE) %>%
  dplyr::select(Precursor.Id, Stripped.Sequence, Protein.Names, Genes) %>%
  dplyr::collect() %>%
  unique()
